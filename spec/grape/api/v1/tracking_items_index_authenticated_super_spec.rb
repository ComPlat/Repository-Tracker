    ...
    context "when trackings exist and user is authorized" do
      ...
      it {
        expect(response.parsed_body.first)
          .to eq JSON.parse(
            API::Entities::TrackingItem.new(tracking_items.first, {tracking_ids: tracking_items.first.tracking_ids}).to_json
          )
      }
      ...
      it {
        expect(response.parsed_body.second)
          .to eq JSON.parse(
            API::Entities::TrackingItem.new(tracking_items.second, tracking_ids: tracking_items.second.tracking_ids).to_json
          )
      }
